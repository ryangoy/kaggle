...
class weird_stepwise_lm:
    def __init__(self, max_nr_covars_per_set = 3, verbose = True):
    ...
    def fit(self, train, y):
        ...
        chosen_covar_sets = list(train.columns)
        ...
        best_mses_by_set = np.zeros(len(covar_pool)-1) + 1e15
        
        # for var in covar_pool
        for i in range(len(covar_pool)-1):
            ci = 0
            mses_per_candidate_covar = []
            #for covar_set in self.chosen_covar_sets:
            for j in range(len(chosen_covar_sets)):
                if i == j: 
                    continue
                if len(chosen_covar_sets[j]) < self.max_nr_covars_per_set:
                    model = linear_model.LinearRegression(fit_intercept = False, 
                        normalize = True, copy_X = True, n_jobs = -1) 
                    model.fit(train[chosen_covar_sets[j] + [covar_pool[i]]], y)
                    # FIXME this is not a good measure of model quality
                    mse = 1-get_reward(y, model.predict(train[covar_set +
                        [var]]))
...
l0_wslm = weird_stepwise_lm( max_nr_covars_per_set = 
